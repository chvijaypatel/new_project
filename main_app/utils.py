...
def generate_link(user,for_,type_):
	...
	if for_ == 'Admin':
		...
		link = 'https://eshopy.xyz/?u='+id_+'&t=a'
		...
		link = 'https://eshopy.xyz/?u='+id_+'&t=u'
		if not UserLinkType.objects.filter(user=user,link_type=type_,link=link).exists():
		   UserLinkType.objects.create(user=user,link_type=type_,link=link,links=id_)
		...
		link = 'https://eshopy.xyz/?u='+id_+'&t=v'
		if not UserLinkType.objects.filter(user=user,link_type=type_,link=link).exists():
			UserLinkType.objects.create(user=user,link_type=type_,link=link,links=id_)
	...
	for x in Vendor.objects.all():
		...
		if Km <= 10:
			results.append({
				'store':x,
				'distance':round(Km, 0),
				'price':Product.objects.filter(store=x.store).order_by('price')[0],
